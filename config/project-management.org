...
* Config navigation
I'm often tweaking this configuration that I need a quick way to jump into an specific section or search quickly for a configuration. For this I created the command down below. This command makes use of =doom-completing-read-org-headings= function to read the headings from =org=  files in a given directory (passed as parameter). This function is also responsible for wiring the completion narrowing functions (=consult=, =vertico=).
