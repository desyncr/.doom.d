...
[[kbd:][SPC n s]] searchs contents in ~org-directory~ by default. I've separated work from personal notes so I have a "personal" org directory and a "work" one. Since ~org~ only allows a single ~org-directory~ I created the functions down below to quickly switch between org directories.
